    ...
    try {
        ...
        if (type === 'daily') {
            ...
            details = `
                ...
                <p>This will process payouts for today's top 3 contestants based on votes received today.</p>
                <p>Payments will be sent via your selected payment method.</p>
            `;
            
            payoutModal.dataset.amount = dailyPayout;
            payoutModal.dataset.amountNaira = payoutNaira;
        } else if (type === 'final') {
            ...
            details = `
                ...
                <p>This will process the final payouts to the top 3 contestants:</p>
                <ul>
                    <li>1st Place: ${formatCurrency(prizePool * 0.5)} (${formatNaira(prizePoolNaira * 0.5)})</li>
                    <li>2nd Place: ${formatCurrency(prizePool * 0.3)} (${formatNaira(prizePoolNaira * 0.3)})</li>
                    <li>3rd Place: ${formatCurrency(prizePool * 0.2)} (${formatNaira(prizePoolNaira * 0.2)})</li>
                </ul>
                <p>Payments will be sent via your selected payment method.</p>
            `;
            
            payoutModal.dataset.amount = prizePool;
            payoutModal.dataset.amountNaira = prizePoolNaira;
        }
...
const processFlutterwavePayment = async () => {
    try {
        ...
        // 1. Create payment records in Firestore
        // 2. Use the Flutterwave API to process payments
        // 3. Update payout status based on payment result
        
        // For demo purposes, we'll simulate a successful payment
        setTimeout(() => {
            hideLoading();
            alert('Payments processed successfully via Flutterwave!');
...
const processSolanaPayment = async () => {
    try {
        ...
        // In a real implementation, you would:
        // 1. Create payment records in Firestore
        // 2. Use the Solana Web3.js API to process payments
        // 3. Update payout status based on payment result
        
        // For demo purposes, we'll simulate a successful payment
        setTimeout(() => {
            hideLoading();
            alert('Payments processed successfully via Solana!');
            payoutModal.style.display = 'none';
